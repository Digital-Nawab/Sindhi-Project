      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {jobs.map((job) => (
            <div
              key={job.id}
              className="bg-orange-50 border rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 p-6 relative"
            >
              <button className="absolute top-4 right-4 bg-red-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md text-sm shadow-md">
                Apply Now
              </button>
              <div>
                ...
                <p className="text-red-500 mb-4">{job.company}</p>
                ...
                <div className="flex flex-wrap gap-2">
                  {job.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-4 py-2 border border-red-500 text-red-500 text-sm rounded-md font-bold bg-white"
                    >
                      {skill}
                    </span>
                  ))}
